...
class Player(BasePlayer):
    ...
    def gpt_process(self, p_vars):
        ...
        if p_vars.get("reason_history_2"):
            _thread_storage_2[p_id] = {"status": "already_finished"}
            return

        history = []
        round_data = {}
        # adjust_payoffs = {}
        phase3_payoff = p_vars.get("phase3_round_payoffs")
        final_payoff = {r: cu(phase3_payoff.get(r)) for r in range(1, C.NUM_ROUNDS + 1)}

        time.sleep((p_vars.get("id_in_subsession", 1) - 1) * 0.5)

        for r in C.reasoning_rounds:
            human_reason = p_vars.get(f"reason_2_{r}", "").strip()
            human_decision = p_vars.get(f'decision_2_{r}')
            is_luckywinner = p_vars.get(f"is_luckywinner_2_{r}")
            ...
            if human_reason and human_decision is not None:
                try:
                    ...
                    if winner_type in ["Human", "Tie"]:
                        ...
                        # adjust_payoffs[r] = cu(C.Pass_Reward) - cu(p_vars.get(f"payoff_2_{r}"))
                        current_round_payoff = cu(C.Pass_Reward) - cu(p_vars.get(f"payoff_2_{r}"))
                        result_text = "您的理由較具體"
                    else:
                        final_payoff[r] = cu(0)
                        # adjust_payoffs[r] = - cu(p_vars.get(f"payoff_2_{r}"))
                        current_round_payoff = - cu(p_vars.get(f"payoff_2_{r}"))
        ...
        total_phase3_payoff = sum(final_payoff.values())

        #self.participant.vars["total_phase3_payoff"] = total_phase3_payoff
        #self.participant.vars["reason_history"] = history

        #self.is_gpt_finished = True
        #self.save()

        _thread_storage_2[p_id] = {
            "status": "done",
            "history": history,
...
class ProcessingPage(Page):
    ...
    def live_method(player, data):
        ...
        if data.get("type") == "check_status":
            global _thread_storage_2
            p_id = player.id_in_subsession

            output = _thread_storage_2.get(p_id)
            ...
            if output:
                ...
                if output.get("status") == "done":
                    player.participant.vars["total_phase3_payoff"] = output["total_phase3_payoff"]
                    player.participant.vars["reason_history_2"] = output["history"]
                    ...
                    if p_id in _thread_storage_2:
                        del _thread_storage_2[p_id]
                    return {player.id_in_group: {"status": "finished"}}
